...
private:
  typedef rb_tree<key_type, value_type, zfwstl::identity<value_type>,
                  key_compare>
      rep_type;
...
public:
  ...
  typedef
      typename rep_type::const_iterator iterator; //!!底层迭代器const_iterator
  ...
  multiset(InputIter first, InputIter last) : t(Compare()) {
    t.insert_equal(first, last);
  }
  template <class InputIter>
  multiset(InputIter first, InputIter last, const Compare &comp) : t(comp) {
    t.insert_equal(first, last);
  }
  multiset(std::initializer_list<value_type> ilist) : t() {
...
public:
  ...
  zfwstl::pair<iterator, iterator> equal_range(const key_type &x) const {
