...
template <class Key, class T,
          class Compare = zfwstl::less<Key> /* ,class Alloc=alloc */>
class multimap {
public:
  typedef Key key_type;
  typedef T data_type;
  typedef T mapped_type;
  typedef zfwstl::pair<const Key, T>
      value_type; // value是可以之后修改的，但是key！！不行
  ...
  private:
     ...
     * 这意味着value_compare对象只能由map类内部创建，这是通过friend
     * class声明实现的， 使得map类可以访问value_compare的私有成员和构造函数
     */
...
private:
  typedef rb_tree<key_type, value_type, zfwstl::select1st<value_type>,
                  key_compare>
      rep_type;
...
public:
  ...
  typedef typename rep_type::iterator
      iterator; // 注意与set区别，因为map的value值不参与红黑树节点排布，因此可改变
  ...
  multimap(std::initializer_list<value_type> ilist) : t() {
  ...
  template <class InputIter>
  multimap(InputIter first, InputIter last, const Compare &comp) : t(comp) {
    t.insert_equal(first, last);
  }
...
public:
  ...
  zfwstl::pair<iterator, iterator> equal_range(const key_type &x) {
