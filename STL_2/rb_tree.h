...
template <class Value> struct __rb_tree_node : public __rb_tree_node_base {
...
struct __rb_tree_base_iterator {
  ...
  void decrement() {
    // NOTE:当 node 是头节点时，递减操作让 node
    // 指向右节点是为了找到树中的最大值节点，即当前节点的前驱节点
  ...
  bool operator==(const __rb_tree_base_iterator &rhs) {
...
struct __rb_tree_iterator : public __rb_tree_base_iterator {
   ...
   * 如果转换失败，dynamic_cast 会返回
   * nullptr（对于指针类型）或抛出异常（对于引用类型）， 因此它比 static_cast
   * 更安全。 例子：Base* b = new Base(); Derived* d =
...
protected:
  ...
  typedef zfwstl::simple_allocator<rb_tree_node /* , Alloc */>
      rb_tree_node_allocator; // 专属之空间配置器，每次分配一个节点大小
public:
...
protected:
  ...
  // TAG: KeyOfValue()这是一个模板函数，它接受一个 value_type
  // 引用类型的对象，并从中提取出一个 Key
  // 类型的键。这个函数通常是一个函数对象或者一个模板函数，它定义了如何从值中提取键
...
public:
  ...
  rb_tree(rb_tree &&rhs) noexcept
      : header(zfwstl::move(rhs.header)), node_count(rhs.node_count),
        key_compare(rhs.key_compare) {
    rhs.header = nullptr;
    rhs.node_count = 0;
  }
  ...
  iterator begin() {
    return iterator(leftmost()); /*使用iterator构造函数*/
  } // RB树的起头为最小/左节点处
  ...
  void erase(iterator hint) {
    auto y = rb_tree_erase_rebalance(hint.node, header->parent, header->left,
                                     header->right);
  ...
  zfwstl::pair<typename rb_tree<Key, Value, KeyOfValue, Compare>::iterator,
               bool>
  insert_unique(const value_type &v) {
    ...
    if (key_compare(
            key(j.node),
            KeyOfValue()(v))) // j指向的节点值小于v值，x新插入点在右子节点
      return zfwstl::pair<iterator, bool>(__insert(x, y, v), true);
  ...
  iterator insert_equal(const value_type &v) {
    ...
    while (x) {
      y = x;
      // 遇“大”则往左，遇“小/等于”则往右
      // TAG: key_compare(KeyOfValue()(v), key(x)) 是一个比较操作，它比较新值 v
      // 的键和当前节点 x 的键
  ...
  equal_range(const key_type &k) const {
    return zfwstl::pair<const_iterator, const_iterator>(lower_bound(k),
                                                        upper_bound(k));
  }
...
private:
  ...
  inline void __rb_tre_rotate_left(__rb_tree_node_base *x,
  ...
  inline void __rb_tre_rotate_right(__rb_tree_node_base *x,
                                    __rb_tree_node_base *&root) {
    // x为旋转点
    ...
    while (x != root && x->parent->color == __rb_tree_red) {
      if (x->parent == x->parent->parent->left) {
        ...
        __rb_tree_node_base *y =
            x->parent->parent->right; // 当前节点x的叔叔节点
        if (y && y->color == __rb_tree_red) {
          ...
          x->parent->color = __rb_tree_black;
          x->parent->parent->color = __rb_tree_red;
          __rb_tre_rotate_right(x->parent->parent,
                                root); // x->parent->parent为旋转点
        }
      } else {
        // 父节点为祖父的右子节点
        ...
        if (y && y->color == __rb_tree_red) {
          ...
          x->parent->color = __rb_tree_black;
          x->parent->parent->color = __rb_tree_red;
          __rb_tre_rotate_left(x->parent->parent,
                               root); // x->parent->parent为旋转点
        }
      }
    }
  ...
  base_ptr rb_tree_erase_rebalance(base_ptr z, base_ptr &root, base_ptr &left_m,
                                   base_ptr &right_m) {
    base_ptr d = z;
    ...
    if (!d->left)
      ...
      r = d->left;
    else { // 右子树的最小节点作为替代节点, 也就是接下来的d的子节点
      d = d->right;
      while (d->left)
        d = d->left;
      r = d->right;
      ...
      while (r != root && (!r || r->color == __rb_tree_black))
        if (r == r_parent->left) {
          ...
          if ((!__w->left || __w->left->color == __rb_tree_black) &&
              (!__w->right || __w->right->color == __rb_tree_black)) {
          ...
          if ((!__w->right || __w->right->color == __rb_tree_black) &&
              (!__w->left || __w->left->color == __rb_tree_black)) {
            __w->color = __rb_tree_red;
            r = r_parent;
            r_parent = r_parent->parent;
          } else {
...
void swap(rb_tree<Key, Value, KeyOfValue, Compare> &lhs,
          rb_tree<Key, Value, KeyOfValue, Compare> &rhs) noexcept {
  lhs.swap(rhs);
}
} // namespace zfwstl
