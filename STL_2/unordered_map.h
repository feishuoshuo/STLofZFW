...
template <class Key, class Tp, class HashFcn = zfwstl::hash<Key>,
          class EqualKey = zfwstl::equal_to<Key>>
class unordered_map {
  typedef hashtable<zfwstl::pair<const Key, Tp>, Key, HashFcn,
                    zfwstl::select1st<zfwstl::pair<const Key, Tp>>, EqualKey>
      ht;
...
public:
  ...
  unordered_map(std::initializer_list<value_type> ilist,
      ...
      : rep(zfwstl::max<size_type>(100, static_cast<size_type>(ilist.size())),
            hf, equal) {
    rep.insert_equal(ilist.begin(), ilist.end());
  }
  template <class InputIter>
  unordered_map(InputIter f, InputIter l) : rep(100, hasher(), key_equal()) {
    rep.insert_equal(f, l);
  }
  template <class InputIter>
  unordered_map(InputIter f, InputIter l, size_type n)
      : rep(n, hasher(), key_equal()) {
    rep.insert_equal(f, l);
  }
  template <class InputIter>
  unordered_map(InputIter f, InputIter l, size_type n, const hasher &hf)
      : rep(n, hf, key_equal()) {
    rep.insert_equal(f, l);
  }
  template <class InputIter>
  ...
  template <class InputIter> void insert(InputIter f, InputIter l) {
    rep.insert_equal(f, l);
  }
  zfwstl::pair<iterator, bool> insert_noresize(const value_type &obj) {
    zfwstl::pair<typename ht::iterator, bool> p =
        rep.insert_equal_noresize(obj);
    return zfwstl::pair<iterator, bool>(p.first, p.second);
  }
  const_iterator find(const key_type &key) const { return rep.find(key); }
  size_type count(const key_type &key) const { return rep.count(key); }
  zfwstl::pair<iterator, iterator> equal_range(const key_type &key) const {
