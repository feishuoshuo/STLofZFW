...
struct __hashtable_iterator {
  typedef zfwstl::hashtable<Value, Key, HashFcn, ExtractKey, EqualKey>
      hashtable;
...
struct __hashtable_const_iterator {
  ...
  const hashtable
      *ht; // 保持对容器的连结关系(因为可能需要从bucket到另一个bucket)
...
public:
  ...
  typedef HashFcn
      hasher; // hash函数的函数型别-->计算元素位置，即bucket位置的函数
  ...
  typedef __hashtable_iterator<value_type, key_type, hasher, ExtractKey,
                               key_equal>
      iterator;
  typedef __hashtable_const_iterator<value_type, key_type, hasher, ExtractKey,
                                     key_equal>
      const_iterator;

public:
  // 提供一个公共的访问器函数
  ExtractKey get_key_function() const { return get_key; }

private:
  // TAG：stl_hash_fun.h
  // 以下三者均为函数对象，stl_hash_fun.h定义有数个标准型别的hasher
  hasher hash;
  key_equal equals;
  ExtractKey get_key;
  typedef __hashtable_node<Value> node;
...
public:
  ...
  template <class Iter,
            typename std::enable_if<zfwstl::is_input_iterator<Iter>::value,
                                    int>::type = 0>
  hashtable(Iter first, Iter last, size_type bucket_count,
            const hasher &hash_ = zfwstl::hash<Value>(),
            const key_equal &equal = zfwstl::equal_to<Value>())
      : num_elements(zfwstl::distance(first, last)), hash(hash_),
        equals(equal) {
    initialize_buckets(zfwstl::max(
        bucket_count, static_cast<size_type>(zfwstl::distance(first, last))));
  }
  hashtable(const hashtable &rhs) : hash(rhs.hash), equals(rhs.equals) {
    copy_from(rhs);
  }
  hashtable(hashtable &&rhs) noexcept
      : num_elements(rhs.num_elements), get_key(rhs.get_key), hash(rhs.hash),
        equals(rhs.equals) {
    buckets = zfwstl::move(rhs.buckets);
    rhs.num_elements = 0;
  }
  ...
  zfwstl::pair<iterator, iterator> equal_range_unique(const key_type &key) {
    ...
    for (node *first = buckets[n]; first; first = first->next) {
      if (equals(get_key(first->val), key)) {
        if (first->next)
          return zfwstl::make_pair(iterator(first, this),
                                   iterator(first->next, this));
        for (auto m = n + 1; m < buckets.size();
  ...
  equal_range_unique(const key_type &key) const {
    ...
    for (node *first = buckets[n]; first; first = first->next) {
      if (equals(get_key(first->val), key)) {
        if (first->next)
          return zfwstl::make_pair(
              const_iterator(first, const_cast<hashtable *>(this)),
              const_iterator(first->next, const_cast<hashtable *>(this)));
        for (auto m = n + 1; m < buckets.size();
             ++m) { // 整个链表都相等，查找下一个链表出现的位置
          if (buckets[m])
            return zfwstl::make_pair(
        ...
        return zfwstl::make_pair(
            const_iterator(first, const_cast<hashtable *>(this)), end());
      }
    }
    return zfwstl::make_pair(end(), end());
  }
  zfwstl::pair<iterator, iterator> equal_range_multi(const key_type &key) {
    const auto n = hashf(key);
    for (node *first = buckets[n]; first; first = first->next) {
      if (equals(get_key(first->val), key)) { // 如果出现相等的键值
  ...
  equal_range_multi(const key_type &key) const {
    ...
    for (node *first = buckets[n]; first; first = first->next) {
      if (equals(get_key(first->val), key)) { // 如果出现相等的键值
        for (node *second = first->next; second; second = second->next) {
          if (!equals(get_key(second->val), key))
            return zfwstl::make_pair(const_iterator(first, this),
        ...
        return zfwstl::make_pair(const_iterator(first, this), end());
      }
    }
    return zfwstl::make_pair(end(), end());
  }
...
public:
  // 打印桶中各个元素的位置
  size_type bucket_index_of(const Key &key) const {
  ...
  void erase(const_iterator position) {
    auto p = position.cur;
    if (p) {
      const auto n = hashf(get_key(p->val));
      auto cur = buckets[n];
      if (cur == p) { // p 位于链表头部
        buckets[n] = cur->next;
        delete_node(cur);
  ...
  void erase(const_iterator first, const_iterator last) {
    if (first.cur == last.cur)
      return;
    auto first_bucket =
        first.cur ? hashf(get_key(first.cur->val)) : buckets.size();
    auto last_bucket =
        last.cur ? hashf(get_key(last.cur->val)) : buckets.size();
    if (first_bucket == last_bucket) { // 如果在 bucket 在同一个位置
      erase_bucket(first_bucket, const_cast<node *>(first.cur),
                   const_cast<node *>(last.cur));
    } else {
      erase_bucket(first_bucket, const_cast<node *>(first.cur), nullptr);
      for (auto n = first_bucket + 1; n < last_bucket; ++n) {
        if (buckets[n] != nullptr)
          erase_bucket(n, nullptr);
      }
      if (last_bucket != buckets.size()) {
        erase_bucket(last_bucket, const_cast<node *>(last.cur));
      }
    }
  }
  ...
  size_type erase_multi(const key_type &key) {
    ...
    if (p.first.cur) {
      size_type tmp = zfwstl::distance(p.first, p.second);
  ...
  void swap(hashtable &rhs) noexcept {
    if (this != &rhs) {
      ...
      zfwstl::swap(num_elements, rhs.num_elements);
      zfwstl::swap(hash, rhs.hash);
      zfwstl::swap(equals, rhs.equals);
      zfwstl::swap(get_key, rhs.get_key);
    }
  }
...
private:
  ...
  void initialize_buckets(size_type n) {
    const size_type n_buckets =
        next_size(n); // next_size()返回最接近 n且 >n 的质数
  ...
  void resize(size_type num_elements_hint) {
    ...
    if (num_elements_hint > old_n) {
      ...
      if (n > old_n) {
        zfwstl::vector<node *> tmp(
            n, static_cast<node *>(0)); // 设置新的buckets
                                        // 处理每一个旧的bucket
        for (size_type bucket = 0; bucket < old_n; ++bucket) {
          node *first = buckets[bucket];
          while (first) {
            size_type new_bucket =
                bkt_num(first->val, n); // 找出节点落在哪个新的bucket内
  ...
  zfwstl::pair<iterator, bool> insert_unique_noresize(const value_type &obj) {
    ...
    for (node *cur = first; cur; cur = cur->next)
      if (equals(get_key(cur->val), get_key(obj)))
        return zfwstl::pair<iterator, bool>(
            iterator(cur, this),
            false); // 若发现与链表中某键值相同，则不插入，立即返回
  ...
  void copy_insert_unique(InputIter first, InputIter last,
    ...
    resize(num_elements +
           zfwstl::distance(first, last)); // 判断是否需要重建表格，如需则扩充
  ...
  void copy_insert_multi(InputIter first, InputIter last,
                         zfwstl::input_iterator_tag) {
    resize(zfwstl::distance(first, last));
    for (; first != last; ++first)
