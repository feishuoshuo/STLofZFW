...
OutputIter unchecked_copy_cat(InputIter first, InputIter last,
                              OutputIter result, zfwstl::input_iterator_tag) {
...
unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
                            ...
                            zfwstl::bidirectional_iterator_tag) {
  while (first != last)
    *--result = *--last;
  return result;
}
...
// 从 first 位置开始填充 n 个值
/**
 * 实际fill_n是一种覆写操作(overwrite),
 * 所以一旦操作区间超越了容器大小，就会造成不可预期的结果
 * 解决办法之一：利用inserter()[iterator
 * adapter]产生一个有能力插入而非覆写的迭代器 如：fill_n(inserter(iv,
 * iv.begin(), 5, 7));
 */
 ...
 * 只有当 Tp和 Up都是1字节大小的整型，且 Tp不是
 * bool类型时，编译器才会编译这个模板实例，
 * 并且返回类型是Tp*（指向Tp类型的指针）
 */
template <class Tp, class Size, class Up>
typename std::enable_if<
    std::is_integral<Tp>::value &&
        sizeof(Tp) == 1 && // 检查Tp是否是整型，且Tp类型大小是否为1字节
        !std::is_same<Tp, bool>::
            value && // 确保Tp不是bool类型，因为bool虽然也是1字节大小，但它的行为可能与其他1字节整型不同
