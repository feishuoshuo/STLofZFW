...
// 容器的首尾，并且新元素已经插入到底部容器的最尾端，调整 heap 大顶堆heap
// 上层元素均大于下层 holeIndex, topIndex均为相对距离
template <class RandomIter, class Distance, class T>
void __push_heap(RandomIter first, Distance holeIndex, Distance topIndex,
                 T value) {
...
template <class RandomIter, class Distance, class T>
void __push_heap_aux(RandomIter first, RandomIter last, Distance *,
                     T *) { // 容器最尾端，即第一个洞号：(last-first)-1
  zfwstl::__push_heap(first, Distance(last - first) - 1,
                      static_cast<Distance>(0), T(*(last - 1)));
}
template <class RandomIter, class Distance, class T, class Compared>
void __push_heap_aux(
    RandomIter first, RandomIter last, Distance *, T *,
    Compared comp) { // 容器最尾端，即第一个洞号：(last-first)-1
  zfwstl::__push_heap(first, Distance(last - first) - 1,
                      static_cast<Distance>(0), T(*(last - 1)), comp);
}
...
void push_heap(RandomIter first, RandomIter last,
               Compared comp) { // 新元素应该已置于底部容器的最尾端
  zfwstl::__push_heap_aux(first, last, distance_type(first), value_type(first),
                          comp);
}
// =====================================pop_heap=================================
...
void __adjust_heap(RandomIter first, Distance holeIndex, Distance len,
                   T value) { // 先进行下溯(percolate down)过程
  auto topIndex = holeIndex;
  auto rchild = 2 * holeIndex + 2;
  while (rchild < len) { // 下溯找到下层较大的子节点
    if (*(first + rchild) <
        *(first + rchild -
          1)) // 默认右子节点较大，但是若左子节点大于右子节点，则rchild指向左子节点
      --rchild;
...
template <class RandomIter, class T, class Distance>
void adjust_heap(RandomIter first, RandomIter last, RandomIter result, T value,
                 Distance *) {
  *result = *first;
...
void __pop_heap_aux(RandomIter first, RandomIter last, T *, Compared comp) {
  zfwstl::adjust_heap(first, last - 1, last - 1, T(*(last - 1)),
