...
private:
  ...
  template <class U>
  static two test(...); // 版本1：兜底版本，接受任意参数（可变参数）
                        // [...为C风格的省略号形参/可变参数]
  template <class U>
  static char test(typename U::iterator_category * = 0); // 版本2：精确匹配版本

public:
  /**
   * 1-如果 T 定义了 iterator_category: 版本2是可调用的（因为 0 可以隐式转换为
   * iterator_category*）
   *
   * 2-如果 T 没有定义 iterator_category: 版本2会
   * SFINAE失败（替换失败不是错误），被编译器忽略。只能选择版本1，返回
   * two，大小为 2 字节
   */
  static const bool value =
      sizeof(test<T>(0)) == sizeof(char); // 表示 T 是否包含 iterator_category
};
...
// 根据迭代器的类别来提供正确的特性
//  使用 std::is_convertible 检查 iterator_category 是否可以转换为
//  input_iterator_tag 或 output_iterator_tag
...
public:
  ...
  // NOTE: Insert
  // Iterators的前进、后退、取值、成员取用等操作都是没有意义的，甚至不被允许
  //(关闭功能)，均返回对象自己
...
public:
  ...
  bool operator==(const self &x) const { return this->current == x.current; }
  bool operator!=(const self &x) const { return this->current != x.current; }
  ...
  // 前进与后退方向完全逆转
  // TAG:
  // 这里的const是一个常量成员函数。这意味着函数不会修改类的任何成员变量，也就是说，它不会改变对象的状态
...
template <class T, class Distance = ptrdiff_t> class istream_iterator {
  friend bool operator==(const istream_iterator<T, Distance> &a,
                         const istream_iterator<T, Distance> &b) {
