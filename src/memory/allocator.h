...
namespace zfwstl {
template <class T> class simple_allocator {
public:
...
public:
  // 内存空间的配置与释放
  static T *allocate() {
    // TAG: static_cast类型转换，将::operator
    // new返回的void*指针转换为T*类型的指针
  ...
  // 对象内容的构造、析构
  static void construct(T *ptr) { zfwstl::construct(ptr); }
  ...
  static void destroy(T *ptr) { zfwstl::destroy(ptr); }
  static void destroy(T *first, T *last) { zfwstl::destroy(first, last); }
};
} // namespace zfwstl
