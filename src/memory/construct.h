 ...
 * destroy   : 负责对象的析构
 */
#include "../../src/iterator.h" // for iterator_traits
...
namespace zfwstl {
// 1.construct 构造对象
template <class T1, class T2> inline void construct(T1 *_p, const T2 &_value) {
  ::new ((void *)_p) T1(_value);
}

template <class T1> inline void construct(T1 *_p) {
 ...
 * 在这种情况下，trivial destructor
 * 函数的特化版本被定义为空函数体，以优化性能，避免不必要的函数调用。
 *
 * 这种特化处理是合理的，因为它避免了对不需要析构的对象进行不必要的函数调用，从而提高了程序的效率。
 * 在实际的 STL 实现中，这种优化可以减少模板实例化的开销，并提高容器操作的性能。
 */
// 版本一：接受一个指针
template <class T1> inline void destroy_one(T1 *_p, std::true_type) {}

template <class T1> inline void destroy_one(T1 *_p, std::false_type) {
  if (_p != nullptr)
    _p->~T1(); // 调用dtor ~T()
}
...
template <class ForwardIter>
inline void destroy_cat(ForwardIter _first, ForwardIter _last,
                        std::false_type) {
  for (; _first != _last; ++_first)
    destroy(&*_first);
}

template <class T> void destroy(T *_p) {
