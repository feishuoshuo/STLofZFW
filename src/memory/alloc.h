...
private:
  ...
  static void *oom_malloc(size_t n) {
    ...
    if (ptr == nullptr) { // 内存分配失败
      if (__malloc_alloc_oom_handler) {
        std::cerr << "oom: malloc error!!" << std::endl;
  ...
  static void *oom_realloc(void *ptr, size_t new_size) {
    ...
    if (new_ptr == nullptr) { // 内存分配失败
      if (__malloc_alloc_oom_handler) {
        std::cerr << "oom: realloc error!!" << std::endl;
        __malloc_alloc_oom_handler(); // 调用 OOM 处理函数
      }
      throw std::bad_alloc(); // 抛出异常
    }
  ...
  static void (
      *__malloc_alloc_oom_handler)(); // 函数指针，所代表的函数将用来处理内存不足情况
...
extern void (*__malloc_alloc_oom_handler)(); // 只声明

// 2. 二级配置器： 为了避免太多小额区块造成的内存碎片
/**
 * 当内存较小时，以内存池管理，每次配置一大块内存，并维护对应的自由链表
 * 如果内存较大，超过 128 bytes，使用一级配置器[即直接调用 std::malloc,
 * std::free]
 */
// 共用体: FreeList
...
private:
  // TAG: volatile
  // 告诉编译器该变量值可能会在程序控制之外随时变化，每次都需重新从内存中读取
...
private:
   ...
   * 这意味着 char
   * 数组的起始地址可以被用作任何其他类型对象的起始地址，而不需要额外的内存对齐操作
  ...
  static char *chunk_alloc(size_t size, size_t &nobj) {
    ...
    // 如果内存池剩余大小连一个区块都无法满足
      ...
      if (pool_bytes >
        ...
        FreeList *volatile *my_free_list =
            free_list + FREELIST_INDEX(pool_bytes);
        // 头插法
      ...
      if (!start_free) { // NOTE: heap 空间也不够，它就去free
                         // list当中找空闲内存填内存池[相当于回收了]
