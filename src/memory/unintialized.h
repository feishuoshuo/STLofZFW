...
#include "../algorithms/algo.h" //for move()
...
ForwardIter uninitialized_copy(InputIter first, InputIter last,
                               ForwardIter result) {
  return zfwstl::__uninitialized_copy(
      first, last, result,
...
template <class ForwardIter, class Size, class T>
inline ForwardIter uninitialized_fill_n(ForwardIter first, Size n,
                                        const T &value) {
  // TAG: std::is_trivially_copy_assignable
  // 用于在编译时检查一个类型是否具有平凡（trivial）的复制赋值操作
  return zfwstl::__uninitialized_fill_n(
      first, n, value,
      std::is_trivially_copy_assignable<
          typename iterator_traits<ForwardIter>::value_type>{});
}
...
ForwardIter uninitialized_move(InputIter first, InputIter last,
                               ForwardIter result) {
  return zfwstl::unchecked_uninit_move(
      first, last, result,
