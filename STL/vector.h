...
public:
  ...
  template <class U> friend void swap(vector<U> &lhs, vector<U> &rhs);
...
public:
  ...
  // 拷贝构造
  vector(const vector &rhs) { range_init(rhs.start, rhs.finish); }
  // 移动构造
  ...
  /**
   * 如果第一个模板参数为 false，则 std::enable_if
   * 不会定义任何东西，这会导致模板的替换失败，但由于 SFINAE，这种失败不会报错
   * ::value 是一个成员，它存储了 is_input_iterator 的编译时计算结果，通常是一个
   * bool 值 这里的 0
   * 是一个整数，它在这里没有实际的语义作用，只是作为一个占位符，表示这个模板特化的存在
   */
  template <class Iter,
  ...
  iterator emplace(const_iterator pos, Args &&...args) {
    ...
    if (finish != end_of_storage && xpos == finish) {
      // TAG: zfwstl::forward<Args>(args)...用于将参数包 args 完美转发给
      // construct 函数，保留每个参数的左值或右值属性
  ...
  iterator erase(iterator position) {
    ...
    data_allocator::destroy(
        finish); // 不用deallocate释放内存，只用析构掉对象即可
    return position;
  }
  // 清空容器
...
private:
    ...
    if (finish != end_of_storage) // 还有备份空间
    {
      ...
      *position = x_copy;
      ...
      try {
        ...
        // 调整水位
        ++new_finish;
        // new_finish = zfwstl::uninitialized_copy(position, finish,
        // new_finish);//将原vector的备用空间中的内容也忠实地拷贝过来
      } catch (...) //... 捕获所有异常的异常处理块
      {
        // commit or rollback
        data_allocator::destroy(new_start, new_finish);
  ...
  // 就地构造元素，涉及是否需要扩容
  template <class... Args> void emplace_aux(iterator position, Args &&...args) {
    if (finish != end_of_storage) // 还有备份空间
    {
      data_allocator::construct(finish, *(finish - 1));
      // 调整水位
      ...
      *position = zfwstl::move(x_copy);
      ...
      try {
        ...
        data_allocator::construct(
            new_finish, zfwstl::forward<Args>(args)...); // 为新元素设定初值 x
        // 调整水位
        ++new_finish;
        // new_finish = zfwstl::uninitialized_copy(position, finish,
        // new_finish);//将原vector的备用空间中的内容也忠实地拷贝过来
