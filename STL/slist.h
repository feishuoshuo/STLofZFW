...
template <class T> struct __slist_node : public __slist_node_base {
  T data;
};
...
struct __slist_iterator : public __slist_iterator_base {
  ...
  } // ((list_node *)node)首先将node指针强制转换为list_node类型的指针
  pointer operator->() const {
    return &(operator*());
  } // iterator->member通常用来访问迭代器当前指向的元素的成员
  self &operator++() {
    incr();       // 向前一个节点
    return *this; // this 是一个指向当前对象的指针，*this
                  // 就是解引用当前对象的指针，得到当前对象的引用
  }
  self &operator++(int) {
    self tmp = *this;
    incr(); // 向前一个节点
...
public:
  ...
  template <class U> friend void swap(slist<U> &lhs, slist<U> &rhs) noexcept;
...
public:
  ...
  slist(const slist &other) {
    head.next = nullptr;
    for (const_iterator current = other.begin(); current != other.end();
         ++current)
      push_back(*current);
  }
  // 移动构造函数
  ...
  slist &operator=(const slist &other) {
    if (this != &other) {
      clear();
      for (iterator current = other.begin(); current != other.end();
           ++current) {
        push_front(*current);
      }
    }
...
//========================模板类外重载操作===============
template <class T> bool operator==(const slist<T> &lhs, const slist<T> &rhs) {
...
// 重载 zfwstl 的 swap
template <class U> void swap(slist<U> &lhs, slist<U> &rhs) noexcept {
  lhs.swap(rhs);
}
} // namespace zfwstl
