...
template <class T, class Container = zfwstl::vector<T>,
          class Compare = zfwstl::less<typename Container::value_type>>
...
public:
  ...
  template <class InputIter,
            typename std::enable_if<zfwstl::is_input_iterator<InputIter>::value,
                                    int>::type = 0>
  ...
  priority_queue(std::initializer_list<T> ilist) : c(ilist) {
    zfwstl::make_heap(c.begin(), c.end(), comp);
  }
  priority_queue(const Container &s) : c(s) {
    zfwstl::make_heap(c.begin(), c.end(), comp);
  }
  priority_queue(Container &&s) : c(zfwstl::move(s)) {
    zfwstl::make_heap(c.begin(), c.end(), comp);
  }
  ...
  void
  swap(priority_queue &rhs) noexcept(noexcept(zfwstl::swap(c, rhs.c)) &&
