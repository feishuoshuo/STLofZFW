...
template <class T, size_t BufSize = 0> struct deque_buf_size {
  // static constexpr size_t value = sizeof(T) < 256 ? 4096 / sizeof(T) : 16;
  static constexpr size_t value =
      (BufSize != 0) ? BufSize : (sizeof(T) < 256 ? 4096 / sizeof(T) : 16);
};
...
struct __deque_iterator : public iterator<random_access_iterator_tag, T> {
  ...
  typedef __deque_iterator /* <T, Ref, Ptr> */
      self;                // 当前迭代器的类型:自引用迭代器
  ...
  static const size_type buffer_size =
      deque_buf_size<T, BufSize>::value; // 缓冲区大小

  // 迭代器所含成员数据 [first, cur)
  // NOTE:cur指向所在缓冲区[1-开头]首元素 ；[2-末尾]尾元素的下一个位置
  value_pointer cur;
  ...
  difference_type operator-(const self &x) const {
    /**
     * node -
     * x.node：计算两个迭代器所在的缓冲区节点之间的差值；由于每个节点代表一个完整的缓冲区，所以要
     * * buffer_size static_cast<difference_type>(buffer_size) * (node -
     * x.node)：计算两个迭代器之间完整缓冲区的元素数量。 (cur - first) - (x.cur
     * - x.first)：计算两个迭代器在各自缓冲区中的相对位置差。
     */
  ...
  // operator+
  // 这个操作不会改变原始迭代器对象的状态，而是创建并返回一个新的迭代器对象
...
//======================================deque===========================================
template <class T /*,class Alloc=alloc*/, size_t BufSize = 0> class deque {
public:
  //  专属空间配置器
  ...
  typedef pointer
      *map_pointer; // 类型：元素的指针的指针(pointer of pointer of T)
...
public:
  ...
  // TAG：以防止类的构造函数参与隐式类型转换。当一个构造函数被声明为 explicit
  // 时，这意味着它不能用于隐式转换，只能用于直接的函数调用和显式构造
  // 参数化构造函数
  ...
  template <class IIter,
            typename std::enable_if<zfwstl::is_input_iterator<IIter>::value,
                                    int>::type = 0>
  ...
  deque(deque &&rhs) noexcept
      : start(zfwstl::move(rhs.start)), finish(zfwstl::move(rhs.finish)),
  ...
  ~deque() {
    if (map != nullptr) {
      ...
      data_allocator::deallocate(
          *start.node,
          start
              .buffer_size); // 表示start.node指针所指向的内存块中的元素数组的首地址
      *start.node = nullptr;
  ...
  void clear() {
    /**
     * deque的最初状态(无任何元素时)包有一个缓冲区，因此
     * clear()完成后恢复初始状态，也一样保留一个缓冲区
     */
    ...
    if (start.node != finish.node) {
      ...
      data_allocator::destroy(
          start.cur,
          finish.cur); // 只有一个缓冲区，仅析构所有元素而不释放缓冲区内存空间
    // 调整状态
    finish = start;
  }
  // 缩减容器容量
  ...
  void shrink_to_fit() noexcept {
    ...
    size_type new_map_size =
        used_buffers + 2; // 保留一个头部和一个尾部的备用缓冲区
    // 分配新的 map
  ...
  template <class... Args> void emplace_front(Args &&...args) {
    {
      if (start.cur != start.first) {
        data_allocator::construct(start.cur - 1,
                                  zfwstl::forward<Args>(args)...);
        --start.cur;
      } else {
  ...
  // NOTE: Args
  // &&...args：这是一个可变参数模板，它允许函数接受任意数量和类型的参数。
  ...
  iterator erase(iterator position) {
    ...
    if (index < (size() >> 1)) {
      zfwstl::copy_backward(start, position,
                            next); // 清除点之前元素比较少，移动清除点之前的元素
      pop_front();
    } else {
      zfwstl::copy(next, finish,
                   position); // 清除点之后元素较少，移动清除点之后元素
      pop_back();
    }
    return start + index;
  }
  iterator erase(iterator first, iterator last) {
    if (first == start && last == finish) { // 清除区间是整个dedque
      clear();
      return finish;
    } else {
      ...
      if (elems_before < (size() - n) / 2) {
        // 前方元素较少
        zfwstl::copy_backward(start, first,
...
protected:
  ...
  // NOTE: deque关键函数！
  // 确保deque有足够的容量来插入新的元素，具体来说，它会根据需要在deque的前端或后端扩展容量
  void require_capacity(size_type n, bool front) {
    if (front && (static_cast<size_type>(start.cur - start.first) < n)) {
      const size_type need_buffer =
          (n - (start.cur - start.first)) / start.buffer_size + 1;
      ...
      create_buffer(start.node - need_buffer, start.node - 1);
               ...
               (static_cast<size_type>(finish.last - finish.cur - 1) < n)) {
      const size_type need_buffer =
          (n - (finish.last - finish.cur - 1)) / finish.buffer_size + 1;
      if (need_buffer >
          static_cast<size_type>((map + map_size) - finish.node - 1)) {
  ...
  void create_map_and_nodes(size_type num_elements) {
    ...
    // 一个map管理节点数最少8个，最多 ="所需节点数
    // +2(前后各预留一个，扩充时可用) "
    map_size =
        zfwstl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), num_nodes + 2);
    ...
    map_pointer cur;
    try {
      // 为 map内的每个现用节点配置buffer缓冲区。所有buffers =
      // deque的可用空间(最后一个buffer可能留有备用空间)
      for (cur = nstart; cur <= nfinish; ++cur)
        *cur = allocate_node();
    } catch (...) {
      //"commit or rollback"
    ...
    if (elems_before < (len / 2)) // 若在容器前半部分插入
    {
      ...
      try {
        if (elems_before >= n) {
          ...
          zfwstl::uninitialized_copy(
              start, start_n, new_begin); // 在新空间的开始位置进行未初始化复制
          start = new_begin;              // 更新容器的开始位置
          zfwstl::copy(start_n, position,
                       old_begin); // 将插入位置前的元素向后移动
          zfwstl::fill(position - n, position, value_copy);
        } else {

          zfwstl::uninitialized_fill(
              zfwstl::uninitialized_copy(start, position, new_begin), start,
              value_copy);   // 在新空间的开始位置进行未初始化填充
          start = new_begin; // 更新容器的开始位置
    ...
    if (elems_before < (len / 2)) {
      ...
      try {
        if (elems_after > n) {
          ...
          zfwstl::uninitialized_copy(
              position, finish, zfwstl::uninitialized_copy(mid, last, finish));
          finish = new_end;
  ...
  void reserve_map_at_back(size_type nodes_to_add = 1) {
    // 若
    // map尾端的节点备用空间不足，则必须重新换一个map(配置更大的，拷贝原来的，释放原来的)
  ...
  void reallocate_map(size_type nodes_to_add, bool add_at_front) {
    ...
    if (map_size >
        2 * new_num_nodes) { // 当前的内存分配是新节点数量的两倍大，说明有足够的空间来添加新节点
      ...
      if (new_nstart < start.node)
        ...
        zfwstl::copy_backward(start.node, finish.node + 1,
                              new_nstart + old_num_nodes);
    } else {
      size_type new_map_size =
          map_size + zfwstl::max(map_size, nodes_to_add) + 2;
      // 配置一块空间给新 map
      map_pointer new_map = map_allocator::allocate(new_map_size);
...
//========================模板类外重载操作===============
// 重载 zfwstl 的 swap
template <class U> void swap(deque<U> &lhs, deque<U> &rhs) noexcept {
  lhs.swap(rhs);
}
...
template <class T> bool operator<(const deque<T> &lhs, const deque<T> &rhs) {
  return zfwstl::lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(),
                                         rhs.end());
}
template <class T> bool operator>(const deque<T> &lhs, const deque<T> &rhs) {
  return rhs < lhs;
