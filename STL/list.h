...
template <class T, class Ref, class Ptr> struct __list_iterator {
  ...
  typedef Ptr pointer;
  /**
   * vector 的迭代器直接使用 T& 作为引用类型，这是因为 vector
   * 存储的是实际的对象，并且这些对象是连续存储的。
   * 因此，迭代器可以直接返回对象的引用。
   *
   * 然而，对于 list（或任何非连续存储的容器，如 set、map 等），情况有所不同，
   * 因为它们通常基于节点和指针来管理元素，而不是直接存储对象.
   * 使用 Ref 作为 reference
   * 类型允许迭代器返回一个引用或代理，而不是对象的实际引用
   */
  ...
  reference operator*() const {
    return (*node).data;
  } //(*node)先解引用得到node指针指向的节点对象，然后.data再访问数据值
...
protected:
  ...
  typedef zfwstl::simple_allocator<list_node /* , Alloc */>
      list_node_allocator; // 专属之空间配置器，每次分配一个节点大小
public:
  //  专属空间配置器
  ...
  template <class U> friend void swap(list<U> &lhs, list<U> &rhs) noexcept;
...
public:
  // 默认构造函数
  list() {
    empty_initialize(0, value_type());
  } // 产生一个空链表 Tip:当中存在一个空白节点
  // 参数化构造函数
  explicit list(size_type n) { empty_initialize(n, value_type()); }
  list(size_type n, const T &value) { empty_initialize(n, value); }
  template <class Iter,
  ...
  // 移动构造
  list(list &&rhs) noexcept : node(rhs.node), size_(rhs.size_) {
    rhs.node = nullptr;
    rhs.size_ = 0;
  }
  ~list() {
  ...
  // 传入一个 new_size，如果链表旧长度大于 new_size 的大小,
  // 那就删除后面多余的节点
  void resize();
  iterator insert(iterator position, const T &x) {
    ...
    static_cast<link_type>(position.node->prev)->next =
        tmp; // 防止该链表就一个节点
...
protected:
  ...
  // 产生(配置并构造)一个节点，带元素值
  template <class... Args> link_type create_node(Args &&...args) {
    link_type p = get_node();
    zfwstl::construct(
        &p->data,
        zfwstl::forward<Args>(args)...); //(&(p->data))取p指向的data成员的地址
    return p;
  }
  // 销毁(析构并释放)一个节点
  ...
  template <class Iter> void copy_assign(Iter f2, Iter l2) {
    auto f1 = begin();
    auto l1 = end();
  ...
  template <class Compared> void _merge(list &x, Compared comp) {
  ...
  template <class Compared> void _sort(Compared comp) {
    ...
    list<T> counter[64]; // 一个包含64个链表的数组
                         // counter，用于基数排序中的桶（bucket）
    int fill = 0;        // 一个计数器, 用于记录 counter 数组中有多少个链表非空
    while (!empty()) {
      carry.splice(carry.begin(), *this,
                   begin()); // 将主链表的第一个元素移动到 carry 链表中
      int i = 0;
  ...
  // 归并排序[_left1, _right2)
  template <class Compared>
  iterator merge_sort(iterator _left1, iterator _right2, size_type n,
                      Compared comp) {
    --_right2;
    if (_left1 == _right2)
      return _left1;
    ...
    auto result = _left1 =
        merge_sort(_left1, _right1, n2, comp); // 前半段的最小位置
    auto _left2 =
        merge_sort(_right1, ++_right2, n - n2, comp); // 后半段的最小位置
...
//========================模板类外重载操作===============
// 重载 zfwstl 的 swap
template <class U> void swap(list<U> &lhs, list<U> &rhs) noexcept {
  lhs.swap(rhs);
}
