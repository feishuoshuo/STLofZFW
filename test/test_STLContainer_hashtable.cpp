...
protected:
  zfwstl::hashtable<int, int, zfwstl::hash<int>, zfwstl::identity<int>,
                    zfwstl::equal_to<int>>
      iht;
    ...
    for (int i = 0; i < iht.bucket_count(); ++i) {
      ...
      if (n != 0) {
        ...
        while (bucket) {
          ...
          auto bucket_index =
              iht.bucket_index_of(key); // 获取键值所在的桶的索引
          std::cout << "  key: " << key << ", value: " << bucket->val
                    << ", count: " << count
                    << ", bucket index: " << bucket_index << std::endl;
          bucket = bucket->next;
        }
      }
    }
