...
TEST_F(SContainerTestList, Swap) {
  ...
  EXPECT_EQ(v3.size(), 5);
  int x = 0;
  for (auto i = v1.begin(), j = v3.begin(); i != v1.end() && j != v3.end();
       ++i, ++j, ++x) {
    EXPECT_EQ(*i, 11 + x);
    EXPECT_EQ(*j, 1 + x);
  }
...
TEST_F(SContainerTestList, RemoveUnique) {
  ...
  v2.remove_if([](int value) { return value % 2 == 0; });
  ...
  print_process("unique: Remove duplicate elements from the list that satisfy "
                "pred as true(Ordered sort)");
  zfwstl::list<int> lst = {1, 2, 1, 3, 4, 5, 5};
  lst.unique([](int a, int b) { return a >= b; });
...
TEST_F(SContainerTestList, Merge) {
  ...
  first2.merge(second2, [](int a, int b) { return a < b; });
...
TEST_F(SContainerTestList, Sort) {
  ...
  lst2.sort([](int a, int b) { return a < b; }, '\0');
