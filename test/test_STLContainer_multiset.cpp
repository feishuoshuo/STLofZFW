...
TEST_F(AContainerTestMSet, Constructor) {
  ...
  zfwstl::multiset<std::string> v5(v4); // 拷贝构造
  EXPECT_EQ(v5, v4);
  EXPECT_FALSE(v4.empty());

  print_process("Move constructor");
  zfwstl::multiset<std::string> v6(zfwstl::move(v4)); // 移动构造
  EXPECT_NE(v6, v4);
  ...
  auto it2 = siset.end();
  zfwstl::multiset<int> v7(it1, it2); // 范围构造函数
  EXPECT_EQ(v7, siset);

  print_process("initializer_list constructor"); // 列表初始化构造
  zfwstl::multiset<std::string> mySet({"one", "two", "three"});
...
TEST_F(AContainerTestMSet, AssignmentOperator) {
  ...
  zfwstl::multiset<int> v4 = {1, 32, 7, 4};
  EXPECT_EQ(v4.size(), 4);

  // 移动赋值操作运算符
  print_process("Move Assignment operator");
  zfwstl::multiset<int> v5 = zfwstl::move(siset);
