 ...
 */
template <class Container> void pri_element(Container &con, std::string name) {
  auto ite1 = con.begin();
  auto ite2 = con.end();
...
TEST_F(AContainerTestUMap, Constructor) {
  ...
  zfwstl::unordered_map<int, const char *> myMap(
      {{1, "one"}, {2, "two"}, {3, "three"}});
...
TEST_F(AContainerTestUMap, AssignmentOperator) {
  ...
  zfwstl::unordered_map<const char *, int> v4 =
      zfwstl::unordered_map<const char *, int>(
          {{"four", 4}, {"five", 5}, {"six", 6}});
  EXPECT_EQ(v4.size(), 3);
  EXPECT_EQ(v4["four"], 4);
  EXPECT_EQ(v4["five"], 5);
...
TEST_F(AContainerTestUMap, InsertErase) {
  ...
  auto res = simap.insert(
      zfwstl::make_pair("zhoufeiwei", 100)); // 在指定位置插入单个元素
  auto i = (*(res)).second;
  EXPECT_EQ(i, 100);
  ...
  zfwstl::unordered_map<const char *, int> v3(
      {{"liuliuilu", 6}, {"egiht", 888}});
